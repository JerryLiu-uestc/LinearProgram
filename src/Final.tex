...
% =========================
\noindent\textbf{摘要：}
本文从线性代数角度探讨图像压缩，比较了快速傅里叶变换（FFT）的矩阵分解与基于奇异值分解（SVD/PAC）的低秩近似，并在 Kodak 数据集上验证了保留奇异值数量 $k$ 对重建质量（PSNR、SSIM）与压缩率的影响。同时指出了浮点存储导致的“负压缩”现象并提出量化处理作为对策。
...
本文以线性代数为主线，先介绍离散傅里叶变换（DFT/FFT）的矩阵表示与分治分解思想，再从统计与代数两视角阐述基于奇异值分解（SVD/PAC）的低秩近似与最优性。随后通过在 Kodak 数据集上的实验评估不同保留秩 $k$ 与不同图像内容下的重建质量与压缩率的权衡，并讨论工程实现中精度与存储格式带来的实际问题。
% =========================================================
% 2. FFT数学原理
% =========================================================
\section{快速傅里叶变换（FFT）}
...
\subsection{实验设置}
实验采用 Kodak Lossless True Color Image Suite 数据集，该数据集包含多幅纹理复杂度各异的高分辨率彩色图像（如 `kodim01.png` 至 `kodim10.png`）。我们设定主成分保留数 $k \in \{5, 10, 20, 40, 80\}$，并采用以下指标进行量化评估：
\begin{itemize}
...
本文以线性代数为视角系统分析了 FFT 与基于 SVD（PAC）的图像压缩方法。理论上，FFT 提供了高效的矩阵分解与基变换框架，适用于频域能量集中的压缩策略；SVD/PAC 则通过截断奇异值实现最优的低秩近似（由 Eckart--Young--Mirsky 定理保证）。实验表明：保留奇异值个数 $k$ 是决定重建质量（PSNR、SSIM）与压缩率之间权衡的关键参数，图像的纹理复杂度会显著影响该权衡；此外，浮点存储精度在高 $k$ 时可能导致“负压缩”，因此在工程实践中需配合合适的量化与编码策略以实现真实的存储收益。未来工作可集中于改进量化与编码方法，并在更多图像类别上验证算法的稳健性。
