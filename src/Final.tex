...
随着图像与多媒体数据量快速增长，如何在保证视觉质量的前提下降低存储与传输成本成为关键问题。许多图像压缩方法的数学内核可以归结为线性变换与矩阵分解：通过基变换将信号表达在频域或其他基下以实现能量聚集，或通过低秩分解提取主要成分以实现有效近似。
 ...
 本文从线性代数角度研究图像压缩，聚焦两类典型方法：基于快速傅里叶变换（FFT）的频域截断，以及基于奇异值分解（SVD/PCA）的低秩近似。我们分别给出 DFT/FFT 的矩阵表示与分治分解解释，并利用截断 SVD 的最优低秩近似性质说明 PCA 压缩的理论依据。随后在 Kodak 数据集上比较不同保留比例/秩下的 PSNR、SSIM 与压缩率（CR），并讨论浮点载荷存储可能导致的“负压缩”等工程现象。
% =========================================================
% 2. FFT数学原理
% =========================================================
\section{快速傅里叶变换（FFT）}
...
\exp\left(-\mathrm{i}2\pi\left(\frac{ux}{H}+\frac{vy}{W}\right)\right),
\end{equation}
...
本文以线性代数为视角系统分析了 FFT 与基于 SVD（PCA）的图像压缩方法。理论上，FFT 提供了高效的矩阵分解与基变换框架，适用于频域能量集中的压缩策略；SVD/PCA 则通过截断奇异值实现最优的低秩近似（由 Eckart--Young--Mirsky 定理保证）。

实验表明：保留奇异值个数 $k$ 是决定重建质量（PSNR、SSIM）与压缩率之间权衡的关键参数，图像的纹理复杂度会显著影响该权衡；此外，浮点存储精度在高 $k$ 时可能导致“负压缩”，因此在工程实践中需配合合适的量化与编码策略以实现真实的存储收益。

当然，我们的算法与主流压缩算法如JPEG、PNG等存在差异，它们通常采用“变换——量化——熵编码”的完整流程，
而本文主要关注线性表示与变换层面。不过，通过本次实验，我们清晰地展示了线性代数在图像压缩中的核心作用，为理解更复杂的压缩算法奠定了坚实的理论基础。
% =========================
% 参考文献
% =========================
